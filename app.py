...
def get_search_term(request):
  return request.form.get('search_term', '')
...
def venues():
  ...
  #   }]
  # }]
  venues_in_city_state = Venue.query.group_by(Venue.id, Venue.city, Venue.state).order_by(Venue.state, Venue.city).all()
  data=[]
  city_state = set()
  i=0
  while i<len(venues_in_city_state):
    city_state.add((venues_in_city_state[i].city, venues_in_city_state[i].state))
    i+=1

  for element in city_state:
      venues_in_city = db.session.query(Venue.id, Venue.name).filter(Venue.city == element[0]).filter(Venue.state == element[1])
      data.append(
        {
        "city": element[0],
        "state": element[1],
...
def search_venues():
  ...
  #print('----->', search_term)
  result = Venue.query.filter(Venue.name.contains(search_term))
  response={
    "count": result.count(),
    "data": result
  }
  #print('result ----->', result)
  #print('responser  ----->', result)
...
def search_artists():
  ...
  # }
  search_term= get_search_term(request)
  result = Artist.query.filter(Artist.name.contains(search_term))
  response={
    "count":result.count(),
    "data": result
  }
...
def create_show_submission():
  ...
  try:
    show = Show()
    newShow = request.form
    show.artist_id = newShow['artist_id']
    show.venue_id = newShow['venue_id']
    show.start_time = newShow['start_time']
    db.session.add(show)
    db.session.commit()
  # on successful db insert, flash success
